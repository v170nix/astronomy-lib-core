...
package net.arwix.astronomy.core.constellation
...
object Constellation {
    private val CONSTELLATION_ABBREV_FOR_LIMITS = arrayOf(
            "And",
            "Ant",
            "Aps",
            "Aql",
    ...
    fun getConsetellation(location: Location): String? {

        val ra = location.longitude * 43200.0 / Math.PI
        val dec = location.latitude * RAD_TO_ARCSEC

        (0..NBNDRIES).forEach { i ->
            var k = i shl 2
            if (ra >= BOUNDARIES[k] && ra < BOUNDARIES[k + 1] && dec > BOUNDARIES[k + 2]) {
                k = BOUNDARIES[k + 3]
                if (k == 74 && location.longitude.toHour() > 17.0) k = CONSTELLATION_ABBREV_FOR_LIMITS.size - 1
    ...
    fun getAllConsetellations() = CONSTELLATION_ABBREV_FOR_LIMITS

}

fun Location.getConsetellation(): String? = Constellation.getConsetellation(this)
